...
proc thomas_diff(i, j, t, ref arr, ref H) {
  ...
  a[1] = 0;
  c[1] = -(2*dt*kappa_v_tmp.localAccess[i,j,1]) / (H.localAccess[i,j,0] * (H.localAccess[i,j,1]+H.localAccess[i,j,0]));
  b[1] = 1 - a[1] - c[1];
  d[1] = arr.localAccess[i,j,t,0];

  a[n] = -(2*dt*kappa_v_tmp.localAccess[i,j,n-1]) / (H.localAccess[i,j,n-1] * (H.localAccess[i,j,n-1]+H.localAccess[i,j,n-2]));
  c[n] = 0;
  b[n] = 1 - a[n] - c[n];
  d[n] = arr.localAccess[i,j,t,n-1];

  for k in 2..<n {
    a[k] = -(2*dt*kappa_v_tmp.localAccess[i,j,k-1]) / (H.localAccess[i,j,k-1] * (H.localAccess[i,j,k-1]+H.localAccess[i,j,k-2]));
    c[k] = -(2*dt*kappa_v_tmp.localAccess[i,j,k]) / (H.localAccess[i,j,k-1] * (H.localAccess[i,j,k]+H.localAccess[i,j,k-1]));
    b[k] = 1 - a[k] - c[k];

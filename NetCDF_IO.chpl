...
proc get_var(filename : string, varName : string, ref arr_in, dom_in) {
...
proc set_bry(filename : string, varName : string, ref arr, dom_in, t) {
...
proc WriteOutput3(ref arr_in, D, varName : string, units : string, i : int) {
  ...
  /* Determine where to start reading file, and how many elements to read */
    // Start specifies a hyperslab.  It expects an array of dimension sizes
      var start = tuplify(D.localSubdomain().first);
      var start_loc = (D.localSubdomain().first[0], D.localSubdomain().first[1], D.localSubdomain().first[2]);
    // Count specifies a hyperslab.  It expects an array of dimension sizes
      var count = tuplify(D.localSubdomain().shape);
      var count_loc = (D.localSubdomain().shape[0], D.localSubdomain().shape[1], D.localSubdomain().shape[2]);
